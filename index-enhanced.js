 ...
 * Upgraded by AI Assistant with comprehensive improvements
 */
...
let userLocksText = {};
let userLocksImage = {};
let userSessions = {}; // Track user interaction state
...
app.get('/', async (req, res) => {
  res.json({
    ...
    features: ["Enhanced UI/UX", "Progress Tracking", "Rich Formatting", "Better Error Handling"]
  })
})
...
bot.onText(/\/ai/, async (msg) => {
  ...
  if (userLocksText[userId]) {
    return bot.sendMessage(msg.chat.id, 
      `${BotUI.WARNING_EMOJI} <b>Please wait</b>\n\n` +
  ...
  if (!input) {
    return bot.sendMessage(msg.chat.id, BotUI.getAIIntroMessage(), { parse_mode: 'HTML' });
  }
  ...
  let processingMsg = await bot.sendMessage(msg.chat.id, 
    ...
    `💭 <b>Question:</b> ${input.substring(0, 100)}${input.length > 100 ? '...' : ''}\n\n` +
    `⏳ <i>Please wait while I generate a response...</i>`, 
    { parse_mode: 'HTML' }
  );
...
// Enhanced Google search with better UI
bot.onText(/\/google/, async (msg) => {
  ...
  if (!input) {
    return bot.sendMessage(msg.chat.id, BotUI.getSearchIntroMessage(), { parse_mode: 'HTML' });
  }
...
// Continue with URL pattern handlers...
// [The rest of the handlers would continue here, but I'll split this into multiple files due to length]
